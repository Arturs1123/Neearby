<template>
    <div v-if="step == 1" class="modal_window">
        <div class="modal_window_i">
            ...
            <span>Вы действительно хотите удалить данную подписку?</span>
            <div class="mw_down">
                <div @click="close" class="btn_no">Нет</div>
                <div @click="deleteSubscription(subscriptionObject.id), step = 2" class="btn_y">Да</div>
            </div>
        </div>
    </div>
    <div v-if="step == 2" class="modal_window">
      <div class="modal_window_i">
        ...
        <span>Ваша подписка была успешно удалёна.</span>
        <div class="mw_down">
...
export default {
    ...
    methods: {
      ...
      async deleteSubscription(productId){
        await ApiWrapper.deleteSubscription({id: productId}).then(() => {
        })
      },
    },
}
