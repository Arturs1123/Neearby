      ...
      <div v-for="el in moderationListed" v-if="step == 1">
        ...
        <div class="moderation-item">
          ...
          <div style="width: 250px; height: 150px; position: relative; left: 15px;">
            <img style="width: 100%; height: 100%;" :src="'https://service-author.neearby.pro/' + el.product_image_path" alt="">
          </div>
...
import FilterPanel from "@/components/AdminPanel/FilterPanel"
...
export default {
  ...
  methods: {
    accept(el) {
      ApiWrapper.acceptProduct({id: el.id, active: 1})
          .then(() => {
            const toast = useToast();
            toast("Вы одобрили товар", {position: POSITION.BOTTOM_RIGHT, type: TYPE.SUCCESS, timeout: 2000});
            ApiWrapper.getModerationProducting()
                .then(res => {
    ...
    cancel(el) {
      ApiWrapper.cancelProduct({id: el.id, active: 2})
          .then(() => {
            const toast = useToast();
            toast("Вы забанили товар", {position: POSITION.BOTTOM_RIGHT, type: TYPE.ERROR, timeout: 2000});
            ApiWrapper.getModerationProducting()
                .then(res => {
                  this.moderationListed = res.data;
                })
    ...
    async openModeration() {
      this.step = 1;
      await ApiWrapper.getModerationProducted()
          .then(res => {
            this.moderationListed = res.data;
          })
    },
    async openModerationAccept() {
      this.step = 2;
      await ApiWrapper.getModerationAcceptProducted()
          .then(response => {
    ...
    async openModerationCancel() {
      this.step = 3;
      await ApiWrapper.getModerationCancelProducted()
          .then(response => {
