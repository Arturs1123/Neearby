  ...
  <div class="product-preview">
    <div style="cursor: pointer;display: contents;">
      <div class="product-image">
        <img :src="product.product_image_path" v-if="product.active == 1" @click="go(product.id)" alt="">
        <img v-else src="../../src/assets/locked.png" alt="">
      </div>

<!--      this.$router.push(`/${product.id}`)-->

      <div class="product-title" v-if="product.active == 2">
        <h3>Заголовок</h3>
      </div>

      <div class="product-title" v-if="product.active == 1">
        <h3>{{ product.title }}</h3>
      </div>
      ...
      <h4 style="margin-left: 10px; text-decoration: line-through; margin-top: 10px;">{{ product.price }}₽</h4>
      <div style="display: flex; flex-direction: row; align-items: center;" class="product-price">
        <h4 style="margin-left: 50px;">{{ product.discount_price }}₽</h4>
        <div
          style="width: 20px; display: flex;color: gray; font-size: 13px; margin-right: 10px; align-items: center; position: relative"
          class="item-icon">
          <img style="height: 15px; margin-left: 30px;"
            src="../assets/bx_link-alt.png" alt=""> 1
        </div>
        <div style="width: 20px; display: flex; color: gray; font-size: 13px; align-items: center;" class="item-icon">
          <img @click="link(product.product_link)" style="height: 15px; margin-left: 30px;"
            src="../assets/bx_link-alt.png" alt="">
          2
        </div>
<!--        <div style="width: 20px; display: flex; color: gray; font-size: 13px; align-items: center;margin-left: 0.5rem;"-->
<!--          class="item-icon">-->
<!--          <img @click="link(product.product_link)" style="height: 15px; margin-left: 30px;"-->
<!--            src="../assets/bx_link-alt.png" alt="">-->
<!--          3-->
<!--        </div>-->
      ...
      <div class="setting">
        <div class="overlay">
          <div class="menu_overlay">
            <button style="border-radius: 10px 10px 0 0" @click="product31 = true">Закрепить</button>
            <a target="_blank" @click="this.$router.push(`/edit-product/${product.id}`)">
              <button>Изменить</button>
            </a>
            <button @click="product34 = true">Удалить</button>
            <button v-if="product.comment_resolution == 2" @click="updateProductCommented(product.id)">Включить комментарии</button>
            <button v-if="product.comment_resolution == 1" @click="product37 = true">Выключить комментарии</button>
            <button style="border-radius: 0 0 10px 10px" @click="doCopy(product.id), product39 = true">Скопировать ссылку на товар</button>
          </div>
        </div>
    ...
    <div class="btns">
      <div class="item-icon" data-title="Лайки" @click="likeProduct(product.id), isLikeProduct = !isLikeProduct">
      ...
      <div v-if="product.comment_resolution === 1" class="item-icon" data-title="Комментарии" @click="productShow = true, step = 2, isLikeComment = true">
      ...
      <div class="item-icon" data-title="Репосты" @click="repostProduct(product.id), isLikeRepost = true">
      ...
      <div class="item-icon-view" data-title="Лайки">
        <img src="../assets/TwentyOffer/ic_views.svg" alt=""> {{ product.likes }}
      </div>
    </div>
<!--    <div class="more_info_pr">-->
<!--      <img src="../assets/Vector_questions_more.svg" alt="">-->
...
<!--  </VueFinalModal>-->
  <VueFinalModal v-model="product31">
    <Product31 :step="step" v-if="product31" @close="product31=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product32">
    <Product32 :step="step" v-if="product32" @close="product32=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product34">
    <Product34 :step="step" v-if="product34" @close="product34=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product35">
    <Product35 :step="step" v-if="product35" @close="product35=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product36">
    <Product36 :step="step" v-if="product36" @close="product36=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product37">
    <Product37 :step="step" v-if="product37" @close="product37=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product38">
    <Product38 :step="step" v-if="product38" @close="product38=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="product39">
    <Product39 :step="step" v-if="product39" @close="product39=false" :product="product" :profile="profile" />
  </VueFinalModal>
  <VueFinalModal v-model="productShow">
    <ProductSocial :step="step" v-if="productShow" @close="productShow=false" :product="product" :profile="profile" />
  </VueFinalModal>
</template>
...
import Product34 from "@/components/ModalsForProducts/Product34";
import Product35 from "@/components/ModalsForProducts/Product35";
import Product36 from "@/components/ModalsForProducts/Product36";
import Product37 from "@/components/ModalsForProducts/Product37";
import Product38 from "@/components/ModalsForProducts/Product38";
import useClipboard from 'vue-clipboard3'
import { copyText } from 'vue3-clipboard'
import Product39 from "@/components/ModalsForProducts/Product39";
import Product31 from "@/components/ModalsForProducts/Product31";
import Product32 from "@/components/ModalsForProducts/Product32";
export default {
  name: "ProductPreview",
  props: {
    product: {
  ...
  setup() {
    ...
    const doCopy = async (productId) => {
      ...
      const text = "http://" + domain + `/` + productId
      try {
  ...
  methods: {
    ...
    async likeProduct(productId){
      await ApiWrapper.likeProduct({productId: productId, img: this.profilePicturePath, name: this.profileName}).then(() => {
        ApiWrapper.getLikesById({target_id: productId, status: 1})
            .then(response => {
              this.likesed = response.data
            })
      })
    },
    async updateProductComments(productId){
      await ApiWrapper.updateProductComments({id: productId, comment_resolution: 2}).then(() => {
      })
    },
    async updateProductCommented(productId){
      await ApiWrapper.updateProductComments({id: productId, comment_resolution: 1}).then(() => {
    ...
    async repostProduct(productId) {
      await ApiWrapper.repostProduct({target_id: productId, target_type: 1, imgUser: this.profilePicturePath, name: this.profileName, imgProduct: this.product.product_image_path}).then(() => {
        window.location.reload();
        ApiWrapper.getRepostById({target_id: productId, active: 1})
            .then(response => {
              this.reposts = response.data
            })
        ApiWrapper.getRepostWallById({user_id: 188, active: 1})
            .then(response => {
              this.reposts = response.data
            })
      });
    ...
    async repostFavor(productId) {
      await ApiWrapper.repostFavor({target_id: this.product.id, target_type: 1, imgUser: this.profilePicturePath, name: this.profileName, imgProduct: this.product.product_image_path}).then(() => {
        ApiWrapper.getFavorById({target_id: this.product.id, active: 1})
            .then(response => {
  ...
  async mounted() {
    await ApiWrapper.getLikesById({target_id: this.product.id, status: 1})
        .then(response => {
          this.likesed = response.data
        })
    await ApiWrapper.getCommentById({target_id: this.product.id})
        .then(response => {
          this.comments = response.data
        })
    await ApiWrapper.getRepostById({target_id: this.product.id, active: 1})
        .then(response => {
          this.reposts = response.data
        })
    await ApiWrapper.getRepostWallById({user_id: 188, active: 1})
        .then(response => {
          this.reposted = response.data
        })
    await ApiWrapper.getFavorById({target_id: this.product.id, active: 1})
        .then(response => {
          this.favors = response.data
        })
