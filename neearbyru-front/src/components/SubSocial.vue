      ...
      <div class="modal_social_i">
        ...
        <div style="overflow: hidden;overflow-y: scroll;height: 400px;" v-if="step == 2">
          ...
          <div class="send_msg">
            ...
            <img src="../assets/TwentyOffer/sendMsg_ic.svg" alt="" @click="commentProduct(subscriptionObject.id)">
          </div>
        </div>
...
export default {
  ...
  props: ['product', 'profile', 'step', 'likes', 'reposts', 'comments', 'subscriptionObject'],
  data() {
    return {
      profileName: '',
      profilePicturePath: '',
  ...
  methods: {
    ...
    async commentProduct(productId) {
      await ApiWrapper.commentSub({subscriptionId: this.subscriptionObject.id, comment: this.comment, img: this.profilePicturePath, user_id: this.userId, name: this.profileName}).then(() =>{
        ApiWrapper.getCommentByIdSub({target_id: this.subscriptionObject.id}).then(response => {
          this.comments = response.data
          this.comment = '';
        })
      });
