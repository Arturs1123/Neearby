<template>
    <div class="wrapper">
        <div class="offer_inner">
            <div class="offer_cont">
                <div class="offer_l">
                    ...
                    <div class="tovar_title">{{ product.title }}</div>
                    <div class="tovar_sup" @click="descriptionShow = true">Показать полное описание товара</div>
                    ...
                    <div class="social_info">
                        <div class="social_info_btns">
                            ...
                            <div v-if="product.comment_resolution === 1" class="si_des comment" data-title="Комментарии" @click="socialShow = true, step = 2, isLikeComment = true">
                ...
                <div class="offer_r">
                    <div class="user_info" v-if="profile.mail">
                        <h3 class="uInfo_title">Вы собираетесь приобрести данный товар через<br> свой аккаунт:</h3>
      ...
      <VueFinalModal v-model="descriptionShow" v-if="descriptionShow">
        <ModalProductBuy @close="descriptionShow=false" :product="product"/>
      </VueFinalModal>
  ...
  props: ['product', 'profile'],
    ...
    methods: {
      ...
      async likeProduct(productId){
        await ApiWrapper.likeProduct({productId: productId, img: this.profile.img, name: this.profile.name}).then(() => {
      ...
      async repostProduct(productId) {
        await ApiWrapper.repostProduct({target_id: productId, target_type: 1, imgUser: this.profile.img, name: this.profile.name, imgProduct: this.product.product_image_path}).then(() => {
      ...
      async repostFavor(productId) {
        await ApiWrapper.repostFavor({target_id: 49, target_type: 1, imgUser: this.profile.img, name: this.profile.name, imgProduct: this.product.product_image_path}).then(() => {
