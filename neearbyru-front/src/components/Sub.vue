<template>
  ...
  <div class="modal" v-if="step==1" style="height: auto">
    ...
    <div class="modal-body">
      ...
      <div>
        <p style="text-align: left; margin-top: 10px" class="text-modal">
  ...
  <div class="modal" v-if="step==2" style="height: auto">
    <div class="modal-header">
      <p style="padding: 25px;"> Модерация подписки </p>
      <img @click="closePhotoModal()" src="../assets/home-view/cancel-modal.svg" alt="">
    ...
    <div class="modal-body">
      ...
      <div>
        <p style="font-size: 17px; line-height: 25px; font-weight: 550; margin-top: 10px;" class="text-modal">
          ...
          подписка была успешна опубликована на Вашу <br>
          страницу в Neearby
        </p>
      </div>
      <div>
    ...
    <div class="modal-body">
      ...
      <div>
        <p style="font-weight: 600; margin-top: 10px" class="text-modal">
          Вы не прошли модерацию Вашей подписки<br>
          Пожалуйста, отредактируйте Вашу подписку. Название <br>
          подписки, Описание подписки, Изображение подписки
        </p>
      </div>
      <div>
        <p style="text-align: left; font-size: 16px; line-height: 20px; color: gray;" class="text-modal">
          Коментарии от модератора: В название подписки нельзя <br>
          указывать нецензурную лексику. В описание подписки <br>
          нельзя обещать бешенные проценты ничего не делая. А <br>
          изображение Вашей подписки не должно нарушать <br>
          авторские права.
        </p>
      </div>
      <div>
        ...
        <button @click="this.$router.push(`/edit-subscription/${productId}`)">Отредактировать подписку</button>
      </div>
...
export  default {
  ...
  mounted() {
    if(window.localStorage.getItem('subId')){
      this.productId = window.localStorage.getItem('subId');
